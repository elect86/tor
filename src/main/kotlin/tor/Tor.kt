...
private class Control(private val con: TorController) {
    ...
    companion object {
        @JvmStatic
        private val EVENTS_HS = listOf("EXTENDED",
                "CIRC",
                "ORCONN",
                "INFO",
                "NOTICE",
    ...
    private fun parsePort(): Int {
        ...
        return socksIpPorts?.find { it.contains(LOCAL_ADDR_FRAGMENT) }?.let {
            Integer.parseInt(it.substring(it.lastIndexOf(":") + 1, it.lastIndex))
        } ?: throw IOException("No IPv4 localhost binding available!")
    }

    val hiddenServices: List<ConfigEntry> get() = con.getConf(HS_OPTS)
    fun enableHiddenServiceEvents() = con.setEvents(EVENTS_HS)
    ...
    fun hsAvailable(onionUrl: String) = con.isHSAvailable(onionUrl.substring(0, onionUrl.indexOf(".")))
...
abstract class Tor @Throws(TorCtlException::class)
protected constructor(protected val context: TorContext, bridgeLines: Collection<String>? = null) {


    private val eventHandler: TorEventHandler = TorEventHandler()
    private val bridgeConfig = bridgeLines?.filter { it.length > 10 } ?: emptyList()
    private val control = try {
        bootstrap()
    } catch (e: Exception) {
    ...
    private fun bootstrap(secondsBeforeTimeOut: Int = TOTAL_SEC_PER_STARTUP, numberOfRetries: Int = TRIES_PER_STARTUP): Control {
        ...
        try {
            for (retryCount in 1..numberOfRetries) {
                ...
                for (secondsWaited in 1..secondsBeforeTimeOut)
                    if (!control.bootstrapped) Thread.sleep(1000, 0)
                    else return Control(control)

                // Bootstrapping isn't over so we need to restart and try again
                control.shutdown()

                /*  Experimentally we have found that if a Tor OP has run before and thus has cached descriptors and
                    that when we try to start it again it won't start then deleting the cached data can fix this.
                    But, if there is cached data and things do work then the Tor OP will start faster than it would if
                    we delete everything. So our compromise is that we try to start the Tor OP 'as is' on the
                    first round and after that we delete all the files. */
    ...
    fun getProxy(streamID: String? = null): Socks5Proxy {
        ...
        streamID?.let {
            ...
            proxy.setAuthenticationMethod(2, { _, proxySocket ->
                ...
                val out = proxySocket.getOutputStream().apply {
                    write(byteArrayOf(1.toByte(), hash.size.toByte()))
                    write(hash)
                    write(byteArrayOf(1.toByte(), 0.toByte()))
                    flush()
                }
                val status = ByteArray(2)
                proxySocket.getInputStream().read(status)
                if (status[1] != 0.toByte()) throw IOException("auth error: " + status[1])
    ...
    internal fun publishHiddenService(hsDirName: String, hiddenServicePort: Int, localPort: Int): HsContainer {
        ...
        synchronized(control) {
            ...
            currentHiddenServices.filter { !it.is_default }.forEach {
                if (it.key == HS_DIR && it.value == hiddenServiceDirectory.canonicalPath)
            ...
            try {
                if (OsType.current.isUnixoid) {
                    val perms = mutableSetOf(Pfp.OWNER_READ, Pfp.OWNER_WRITE, Pfp.OWNER_EXECUTE)
            ...
            config += "$HS_DIR ${hostnameFile.parentFile.canonicalPath}"
            config += "$HS_PORT $hiddenServicePort $LOCAL_IP:$localPort"
    ...
    fun unpublishHiddenService(hsDir: String) {
        synchronized(control) {
            ...
            for (service in currentHiddenServices) {
                ...
                conf += "${service.key} ${service.value}"
            }
    ...
    fun isHiddenServiceAvailable(onionUrl: String) = control.hsAvailable(onionUrl)

    /** Returns the root directory in which the Tor Onion Proxy keeps its files. This is mostly intended for debugging purposes.
     *  @return Working directory for Tor Onion Proxy files     */
    val workingDirectory get() = context.workingDirectory

    fun shutdown() = synchronized(control) { control.shutdown() }
}
