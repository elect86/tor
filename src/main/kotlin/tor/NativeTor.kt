...
class NativeTor @Throws(TorCtlException::class)
constructor(workingDirectory: File, bridgeLines: Collection<String>? = null) : Tor(NativeContext(workingDirectory), bridgeLines)


class NativeContext(workingDirectory: File) : TorContext(workingDirectory) {

    override val processId get() = java.lang.management.ManagementFactory.getRuntimeMXBean().name.split("@")[0]

    override val pathToTorExecutable by lazy {
        when (OsType.current) {
    ...
    override val pathToRC = "$rcPath$FILE_TORRC_NATIVE"

    override val torExecutableFileName by lazy {
        when (OsType.current) {
    ...
    override fun getAssetOrResourceByName(fileName: String) =
            this::class.java.getResourceAsStream("/$fileName") ?: throw IOException("Could not load $fileName")

    override fun generateWriteObserver(file: File): WriteObserver = NativeWatchObserver(file)

    override fun installFiles() {
        super.installFiles()
        if (OsType.current == OsType.ANDROID)
            throw RuntimeException(OS_UNSUPPORTED)
        else
            extractContentFromArchive(workingDirectory, getAssetOrResourceByName(pathToTorExecutable + FILE_ARCHIVE))
    }
}
